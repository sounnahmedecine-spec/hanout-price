...
function Hit({ hit }: { hit: PriceRecord & { objectID: string } }) {
  ...
  return (
     <div className="grid grid-cols-[auto,1fr,auto] items-center gap-4 p-4 hover:bg-secondary transition-colors rounded-lg">
      <div 
        className="w-12 h-12 rounded-md bg-muted flex-shrink-0 relative cursor-pointer"
        onClick={() => router.push(`/prices/${hit.objectID}`)}
      >
        {hit.imageUrl ? (
      ...
      <div className="flex flex-col items-end gap-1">
        <p 
            className="font-semibold text-lg text-primary cursor-pointer"
            onClick={() => router.push(`/prices/${hit.objectID}`)}
        >
