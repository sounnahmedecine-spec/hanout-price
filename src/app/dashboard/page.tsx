          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            {products.map((product) => (
              <ProductCard
                key={product.id}
                record={{
                  id: product.id, // Assure que l'ID est bien passé
                  imageUrl: product.image,
                  price: parseFloat(product.price), // Convertit la chaîne en nombre
                  productName: product.name, // "Traduit" name en productName
                  storeName: product.store,   // "Traduit" store en storeName
                  userId: 'mock-user',
                  timestamp: Timestamp.fromDate(new Date()), // Crée un objet Timestamp
